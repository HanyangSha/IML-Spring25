...
int main() {
    ...
    if (n == 10) input_filename = "connected/graph10c.g6";
    setIn(input_filename); 
    srand(time(NULL)); 

    auto start = high_resolution_clock::now();

    vector<pll> res(n,mp(0,0)); 
    F0R(i,num[n]) {
    ...
    FOR(k,1,n) {
        double t = res[k].ss;
        double percentage = (double)(res[k].ff) / t; 
        double variance = (double)(res[k].ff)*(1.0-percentage)*(1.0-percentage)/t; 
        double bound = sqrt(2*variance*x/t) + 3*1*x/t; 
        printf("n=%d, k=%d, percentage solvable = %.4f, variance = %.4f, error bound = %.4f\n", n, k, percentage, variance, bound); 
    }
